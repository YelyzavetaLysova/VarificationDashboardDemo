...
export default function UploadForm({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 🔥 Remove the manual Content-Type override so axios
      //    can insert the proper boundary for multipart/form-data
      const res = await axios.post(apiUrl, formData);

      onUploadSuccess?.(res.data);
    } catch (err) {
      console.error("Upload error:", err.response?.data ?? err);
      // show the first validation message if there is one:
  ...
  return (
    <form action="http://127.0.0.1:8000/upload/?request=111" method="POST" encType="multipart/form-data"
